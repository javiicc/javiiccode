...
> 📝 In parallelism two things occur in overlapping time intervals and both occur 
> simultaneously, while in concurrency without parallelism they occur in overlapping time 
> intervals but they don&#39;t occur simultaneously, instead they are swicthed one for the other 
> very quickly.
...
- 1 process, **1 thread**, 1 core
    - ❌ No concurrence with multithreading. There are no multiple threads with which 
    to do context switches, so there is no concurrence.
    - ✅ Concurrence with coroutines (Async).
- 1 process, **n threads**, 1 core
    - ✅ Concurrence, we can switch between threads in the same core.
    - ❌ No parallelism (hardware does not allow parallelism, except with Hyper-Threading).
- 1 process, **n threads**, 2 cores
    - ✅ Concurrence.
    - ✅❌ Parallelism or not depending on programming language 
...
the “normal” intrucctions are represented as filled circles.
...
and make an even better allocation of resources ✨
...
Executing them concurrently won’t offer any improvement in the time taken to be 
