...
> 📘 If you haven't yet, I would recommend you to read my article on 
...
them is the *main thread*. 

~~When a developer writes a program, he can create several threads to execute different 
things (those threads are called *user threads*, and they are created on the *user space*). ???~~
...
~~Also, if the machine has two processes running, it will have concurrence since both have at least one thread. So there are two threads that can leverage the single core.~~
...
### Thread 1 - 1 IO-bound op of 1s and 1 CPU-bound op of 1s aprox Thread 2 - 1 CPU-bound task
...
### 1 thread - 10 IO-bound operations of 1s sequentially
